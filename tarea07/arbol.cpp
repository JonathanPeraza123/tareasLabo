...
void Abb::buscar(){
    int n;
    cout << endl <<"Digite nodo a buscar:" << endl;
    cin >> n;
    bool search = busqueda(pAbb, n);
    if(search){
...
bool Abb::busqueda(nodo *arbol, int n){
    if(arbol == NULL){
        return false;
    }else if(arbol->dato == n){
        return true;
    } else if (n < arbol->dato){
        return busqueda(arbol->hijoizq, n);
    } else {
        return busqueda(arbol->hijodere, n);
    }
}
